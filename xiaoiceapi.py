...
class xiaoiceApi():
    ...
    def loop(self, input_strs):
        ...
        while times <= 20:
            times += 1
            url = "https://weibo.com/aj/message/getbyid?ajwvr=6&uid=5175429989&count=1&_t=0"
            #同上，原http的api已改为https的api,另外headers用全反而无法获取页面，只需用到cookies
            response = requests.get(url, headers={"Cookie":self.headers["Cookie"]})
    ...
    def api(self):
        ...
        @app.route("/")
        def index():
            # decode 是为了处理 node 层发出的请求
            que = request.args.get("que").decode("unicode-escape")
            print('>>>>>>>>', que);
            ans = self.chat(que)
            return jsonify(ans)
        app.run()
